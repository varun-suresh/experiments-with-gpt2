...
class PatchEmbedding(nn.Module):
    def __init__(self, config):
        ...
        self.conv = nn.Conv2d(
            3, config.d_emb, kernel_size=config.patch_size, stride=config.patch_size
        )
...
class MultiHeadedAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        B, ns, d = (
            x.size()
        )  # Batch size, sequence length, embedding dimension(same as config.d_emb)
        q = self.query(x)
        k = self.key(x)
        v = self.value(x)
        q = q.view(B, ns, self.config.n_heads, d // self.config.n_heads).transpose(
            1, 2
        )  # (B,ns,n_heads,d_k) -> (B,n_heads,ns,d_k)
        k = k.view(B, ns, self.config.n_heads, d // self.config.n_heads).transpose(
            1, 2
        )  # (B,ns,n_heads,d_k) -> (B,n_heads,ns,d_k)
        v = v.view(B, ns, self.config.n_heads, d // self.config.n_heads).transpose(
            1, 2
        )  # (B,ns,n_heads,d_k) -> (B,n_heads,ns,d_k)
        attn = (
            nn.functional.softmax(
                q @ k.transpose(-2, -1) / math.sqrt(d / self.config.n_heads), dim=-1
            )
            @ v
        )
...
class VisionTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.position_embedding = nn.Parameter(
            torch.randn(1, config.num_patches + 1, config.d_emb)
        )
