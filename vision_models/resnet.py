    ...
    def __init__(
        ...
        if block_number == 0:
            self.convBlock1 = convBlock(
                input_channels, intermediate_channels, 1, stride=stride
            )
        ...
        self.convBlock2 = convBlock(
            intermediate_channels, intermediate_channels, 3, padding=1
        )
...
class BottleNeckBlock(nn.Module):
    def __init__(
        ...
        self.block = nn.ModuleList(
            BottleNeckLayer(
                input_channels, intermediate_channels, output_channels, i, stride
            )
...
class ResNet50(nn.Module):
    def __init__(self, config):
        ...
        self.fc = nn.Conv2d(
            config.block5.output_channels, config.n_classes, kernel_size=1
        )
...
class conv2Block(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size, stride=1, projection=False
    ):
...
class conv2nBlock(nn.Module):
    def __init__(self, n, in_channels, out_channels, kernel_size, stride):
        ...
        self.blocks = nn.ModuleList(
            conv2Block(out_channels, out_channels, kernel_size) for _ in range(n - 1)
        )
