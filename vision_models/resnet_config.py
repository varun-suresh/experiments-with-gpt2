...
class ResNet50Config:
    ...
    conv1 = convConfig(
        input_channels=3, output_channels=64, kernel_size=7, stride=2, padding=3
    )
    maxpool1 = maxpoolConfig(kernel_size=3, stride=2, padding=1)
    block2 = BottleNeckBlockConfig(
        input_channels=64,
    ...
    block3 = BottleNeckBlockConfig(
        input_channels=256, intermediate_channels=128, output_channels=512, n_blocks=4
    )
    block4 = BottleNeckBlockConfig(
        input_channels=512, intermediate_channels=256, output_channels=1024, n_blocks=6
    )
    block5 = BottleNeckBlockConfig(
        input_channels=1024, intermediate_channels=512, output_channels=2048, n_blocks=3
    )
    n_classes: int = 1000
