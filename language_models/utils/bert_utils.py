...
def pad_sentences(s,max_length):
    length = min(max_length, max([len(x.input_ids[0]) for x in s]))
    input_ids = [x.input_ids[0][0:length] for x in s]
    token_type_ids = [x.token_type_ids[0][0:length] for x in s]
    attention_mask = [x.attention_mask[0][0:length] for x in s]
    # print(input_ids)
...
def dynamic_padding_scbert(data,max_length=512):
    s1 = [tokenizer(item["review"],return_tensors="pt") for item in data]
    s1_padded = sentence(**pad_sentences(s1,max_length))
    label = torch.tensor([item["label"] for item in data])
